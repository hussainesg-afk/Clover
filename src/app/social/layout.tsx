...
function SocialLayoutInner({ children }: { children: React.ReactNode }) {
  ...
  const isMessages = pathname.startsWith("/social/messages");

  return (
    <div>
      <div className="mb-6">
        <h1 className="text-2xl font-bold text-stone-900">Social</h1>
        <p className="mt-1 text-stone-600">Friends and direct messages.</p>
      </div>

      <div className="flex min-w-0 max-w-full flex-col gap-6 lg:flex-row lg:gap-8">
        <nav className="flex max-w-full shrink-0 gap-2 overflow-x-auto pb-2 lg:flex-col lg:overflow-visible lg:pb-0">
        ...
        <div
          className={`min-w-0 max-w-full flex-1 rounded-2xl bg-white shadow-sm ring-1 ring-stone-200/80 ${
            isMessages ? "overflow-x-hidden" : "p-6"
          }`}
        >
          {children}
        </div>
